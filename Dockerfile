...
RUN /etc/init.d/mysql start && sleep 10s && echo "GRANT ALL PRIVILEGES ON *.* TO tao@localhost IDENTIFIED BY 'taopass'; FLUSH PRIVILEGES;" | mysql
RUN sed -i '$i/etc/init.d/mysql start' /usr/local/bin/apache2-foreground
...
RUN cd /var/www/html && sudo -u www-data php /tmp/composer.phar install
RUN cd /var/www/html && curl -O https://hub.taotesting.com/resources/taohub-articles/articles/third-party/MathJax_Install_TAO_3x.sh && chmod u+x MathJax_Install_TAO_3x.sh && ./MathJax_Install_TAO_3x.sh
